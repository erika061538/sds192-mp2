...
### Topic
Since the 2000 United States presidential election, red states and blue states have referred to states of the United States whose residents predominantly vote for either the Republican Party (red) or Democratic Party (blue) presidential candidates. A purple state refers to a swing state where both Democratic and Republican candidates receive strong support without an overwhelming majority of support for either party. For this project, we will observe the different trends amongst the states.

We will use the `candidates` and `house_elections` tables, looking at the number of candidates over the years, between Democratic and Republican Party, and among states. We also want to see which party that states support. 
