...
### Topic
We are interested in looking at the number of candidates running for house elections over the years, of Democratic and Republican Party, and among states.
...
We wrote a function to filter candidates who ran for house elections based on their party affiliation rather than re-writing the same code multiple times. Verbs used: `filter` and `select`.
...
```
A way to plot the number of candidates running for house elections based on parties 
...
Plot the result for the candidates in Democratic Party by states. We can see some main states are CA, IL, NY, TX.
```{r}
candidates_state_dem <- candidates %>%
  filter(cand_party_affiliation == "DEM") %>%
  select(cand_id, cand_name, cand_party_affiliation, cand_office_state) %>%
  group_by(cand_office_state, cand_party_affiliation) %>%
  summarize(num_cand = n())
candidates_state_dem

myPlot3 <- ggplot(candidates_state_dem, aes(x=cand_office_state,y=num_cand))+geom_bar(stat = "identity")
